...
def hexplot_2features(x_idx, y_idx, model_fname_list, m_f_mat, mat_mask):
    ...
    hexplot = sns.jointplot(x=x, y=y, kind="hex", bins="log", mincnt=1, marginal_kws=dict(bins=50, color="black"),
                            joint_kws={"color": None, "cmap": 'viridis'})
    hexplot.set_axis_labels(model_fname_list[x_idx], model_fname_list[y_idx], fontsize=14)
    ...
    plt.text(0.2, y.max(), "R2= {}".format(r_squared_rounded), fontsize=10, horizontalalignment='center',
             verticalalignment='center',
             bbox=dict(facecolor='white', edgecolor='black', boxstyle='round'))

    # make new ax object for the cbar and add colorbar to it
    cbar_ax = hexplot.fig.add_axes(
        [.85, 0.2, .025, .5])  # x, y, width, height (adds an axes where the  colorbar will be drawn)
    plt.colorbar(cax=cbar_ax)  # cax specifies the axes onto which colorbar will be drawn
...
def pairplots_allfeatures(model_fname_list, m_f_mat, mat_mask):
    ...
    # Make plots
    sns.pairplot(df, kind='hist', diag_kind='kde', corner=True, diag_kws=dict(fill=False),
                 plot_kws=dict(color=None, cmap="viridis", cbar=True, norm=LogNorm(), vmin=None, vmax=None))
