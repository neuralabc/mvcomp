...
def mah_dist_mat_connectivity(feature_mat, pinv_s):
    ...
    dist_mem = np.memmap(tf, dtype='float16', mode='w+', shape=(feature_mat.shape[0], feature_mat.shape[0]))
    ...
    for i in range(feature_mat.shape[0]):
        ...
        if i % 100 == 0:
            ...
            f = np.concatenate((f, np.tile(feature_mat[i, :], (feature_mat.shape[0], 1))), axis=0)
        ...
        if i == feature_mat.shape[0] - 1:
            ...
            dist_1 = da.dot(matrix_multi, da.ones((feature_mat.shape[1], 1), dtype='float16'))

            dist_mem[i - (feature_mat.shape[0] - 1) % 100:feature_mat.shape[0]] = np.reshape(dist_1, (-1, n))
...
def connectivity_map(feature_in_dir, model_dir, suffix_name="warped_multcon_feature.nii.gz", mask_f=None, mask_img=None,
                     mask_threshold=0.9, subject_ids=None, feat_sub=[]):
    ''''
    ...
    # Create a list of the features from the model. htis list contains the location address of the features.
    model_feature_image_fname_list, model_f_list = feature_list(model_dir, suffix_name, feat_sub)

    # create feature matrix from the model
    m_f_mat, mask_img, mat_mask = feature_gen(model_feature_image_fname_list, mask_image_fname=mask_f,
                                              mask_image=mask_img, mask_threshold=0.9)
    ...
    all_dist = np.zeros((m_f_mat.shape[0], m_f_mat.shape[0], len(subject_ids)), dtype="float16")
    ...
    for idx, subject_id in enumerate(subject_ids):
        ...
        for f_name in model_f_list:
            if os.path.isfile(feature_in_dir + subject_id + "/" + f_name + suffix_name):
                comp_image_fname_list.append(feature_in_dir + subject_id + "/" + f_name + suffix_name)
        ...
        if not (model_f_list == comp_f_list):
            print(
                ">>You do not have exactly the same metric names for your model and comparison images (stopping)<<\n\tmodel:\t{}\n\tcomp:\t{}".format(
                    model_f_list, comp_f_list))
            break
        else:  # everything is OK! lets do the comparison!
            c_f_mat, _, sub_mat_mask = feature_gen(comp_image_fname_list, mask_image=mask_img,
                                                   mask_threshold=0.9)  # extract features from each individual

            all_feat[..., idx] = c_f_mat
            all_mask[..., idx] = sub_mat_mask
            print("subject ", subject_id, "feature matrix creation in ", time.time() - st, "s")
    ...
    for idx in range(num_subs):  # for each subject
        ...
        # do MHD
        all_dist[..., idx] = mah_dist_mat_connectivity(all_feat[..., idx], pinv_s)
...
def gen_roi_feature_vec_mat(feature_image_fname_list, region_roi_mask, summary_metric='mean'):
    """
...
def precomputed_spatial_mvcomp(subjects_matrix, model_feature_vector, pinv_s, return_raw=False):
    '''
