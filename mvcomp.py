...
def feature_list(feature_in_dir, suffix_name, remove_list=[]):
    """
    This function is to create a list of the features names (and of their full paths) from the reference (model)
    
    Args:
        1. feature_in_dir (String): defines the directory where the features are located
        2. suffix_name (String): defines the suffix that comes after the features name (e.g. in "T1_divided_T2_mean.nii.gz" the suffix is "_mean.nii.gz"
        3. remove_list (List of strings): A list of the features names we want to exclude from calculation
    
    Returns:
        1. feature_image_fname_list (List of strings): A list of the full pathnames of the features.
...
def feature_gen(feature_image_fname_list, feature_in_dir=None, mask_image_fname=None, mask_image=None, verbosity=0,
    ...
    for idx, feature_image_fname in enumerate(feature_image_fname_list):
        feature_img = nb.load(os.path.join(feature_in_dir, feature_image_fname))
        if (mask_image_fname is not None) and (mask_image is None):
            ...
            if (idx == 0):
                if verbosity > 0:
                    print("Generating a mask from the first input feature image, where values == 0")
    ...
    feature_mat_vec_mask += np.isinf(feature_mat).sum(axis=-1)  # this will NOT be binary
    feature_mat_vec_mask = np.logical_not(feature_mat_vec_mask.astype(bool))  # now we have zeros where the nan and infs are
...
def mah_dist_feat_mat(feature_mat1, feature_mat2, pinv_s, return_raw=False):
    """
    Computes the Mahalanobis distance (D2) between two feature matrices with the option of returning raw distances.
    Args:
    ...
    Returns: 
        results (numpy array): A 1D array of size (number of voxels) containing D2 values between two feature matrices.
        Or
...
def subject_list(root_dir, ex_subjects=[]):
    ...
    Args:
        root_dir (string): Address of the working directory that contains all the subjects (The subjects folder names should be all number e.g. "001020").
        ex_subjects (List of strings): A list of subjects to be excluded from the subjects list.
        
    Returns:
        subject_ids (List of strings): A list of all the subjects inside the root_dir, except the ones specified in ex_subjects.
...
def model_comp(feature_in_dir, model_dir=None, suffix_name_comp=".nii.gz", exclude_comp_from_mean_cov=True,
    ...
    if not exclude_comp_from_mean_cov: #if we don't care that our comparison is within the mean, then we can compute this one time
        ...
        if return_raw:
            ...
            raw_dist = np.zeros((m_f_mat.shape[0],m_f_mat.shape[1], len(subject_ids)))
    ...
    for idx in range(num_subs):  # for each subject
        if exclude_comp_from_mean_cov: #we remove the subject that is going to be compared from the mean (reference) and pinv_s calculation so that they are independent
            ...
            s, pinv_s = norm_covar_inv(m_f_mat[:,], np.sum(all_mask,axis=-1)==num_subs) #mask specific to voxels from ALL subjects

        # compute D2
        raw_dist[..., idx] = mah_dist_feat_mat(all_feat[..., idx], m_f_mat, pinv_s, return_raw=return_raw)
    
    print("Total time for mahalanobis distance calculation on {} subjects with {} voxels: {:.3}s".format(num_subs, sub_mat_mask.shape[0], time.time() - st))

    # if we set return_raw=True then we still need to compute D2 by summing across the features of the 3d array returned by mah_dist_feat_mat
...
def dist_plot(all_dist, all_mask, subject_ids, feat_sub=[], save_results=True, out_dir=None, mask_f=None, mask_img=None,
    ...
    oob_mask = np.prod(all_mask, axis=-1) == 0  # create the mask of nan and inf values
    ...
    allmask_out = nb.Nifti1Image(mask_out, affine=mask_img.affine, header=mask_img.header)
    ...
    if save_results:
        ...
        if len(feat_sub) != 0:
            if not os.path.isdir(result_dir + "results_without" + feat_str + "_" + str(len(subject_ids))):
                os.makedirs(result_dir + "results_without" + feat_str + "_" + str(len(subject_ids)))
            result_dir = result_dir + "results_without" + feat_str + "_" + str(len(subject_ids))
        else:
            if not os.path.isdir(result_dir + "results_with_allfeatures" + "_" + str(len(subject_ids))):
        ...
        for idx, subject_id in enumerate(subject_ids):
            d_out = all_dist[:, idx]
            d_out[all_mask[:, idx] == 0] = 0  # just zero the values that are fringe
...
def model_comp_simplified(comp_images_fname_list,subject_ids=None,model_feature_list=None,model_feature_image_fname_list=None, 
    ...
    When model_feature_image_fname_list == None, model mean and covariance is generated by leaving out all but the current subject being compared.
    When model_feature_image_fname_list is provided, mean and covariance are based on the specified files. 

    Args:
        comp_images_fname_list (list): list of lists, where the first dimension (outer) is subject and second (inner) 
    ...
    if not exclude_comp_from_mean_cov: #if we don't care that our comparison is within the mean, then we can compute this one time
        ...
        if return_raw:
            ...
            raw_dist = np.zeros((m_f_mat.shape[0],m_f_mat.shape[1], num_subjects))
            all_mask = np.zeros((m_f_mat.shape[0], num_subjects))

        else:
            all_feat = np.zeros((m_f_mat.shape + (num_subjects,)))
            raw_dist = np.zeros((m_f_mat.shape[0], num_subjects)) #actually will just contain the distnaces, not the raw weightings
            all_mask = np.zeros((m_f_mat.shape[0], num_subjects))
    ...
    for idx, comp_image_fname_list in enumerate(comp_images_fname_list):
        ...
        if idx == 0:
            if exclude_comp_from_mean_cov: #we have not defined the output matrices yet in this case, so define here
                if return_raw:
                    all_feat = np.zeros((sub_mat_mask.shape[0],num_features,num_subjects))
                    raw_dist = np.zeros((sub_mat_mask.shape[0],sub_mat_mask.shape[1],num_subjects))
                    all_mask = np.zeros((sub_mat_mask.shape[0],num_subjects))
                else:
                    all_feat = np.zeros((sub_mat_mask.shape[0],num_features,num_subjects))
                    raw_dist = np.zeros((sub_mat_mask.shape[0],num_subjects)) #actually will just contain the distnaces, not the raw weightings
                    all_mask = np.zeros((sub_mat_mask.shape[0],num_subjects))
        all_feat[..., idx] = c_f_mat
        all_mask[..., idx] = sub_mat_mask
        if verbosity >=1:
            print("subject {} feature matrix creation in {:.3} s".format(subject_ids[idx], time.time()-st))

    st = time.time()
    for idx in range(num_subjects):  # for each subject
        if exclude_comp_from_mean_cov: #we remove the subject that is going to be compared from the mean and pinv_s calculation so that they are independent
            m_f_mat = np.mean(np.delete(all_feat,idx,axis=-1),axis=-1)
            s, pinv_s = norm_covar_inv(m_f_mat[:,], np.sum(all_mask,axis=-1)==num_subjects) #mask specific to voxels from ALL subjects

        # compute D2 
        raw_dist[..., idx] = mah_dist_feat_mat(all_feat[..., idx], m_f_mat, pinv_s, return_raw=return_raw)
    
    print("Total time for mahalanobis distance calculation on {} subjects with {} voxels: {:.3}s".format(num_subjects, sub_mat_mask.shape[0], time.time() - st))
...
def spatial_mvcomp(subjects_matrix, return_raw=False, exclude_comp_from_mean_cov=False):
    '''
    To compute D2 between subjects and a reference, where dimensions combined are spatial dimensions (e.g., WM tracts or ROIs).
    Args:
    ...
    if exclude_comp_from_mean_cov == False:
        s, pinv_s = norm_covar_inv(subjects_matrix) #compute tract-wise covariance
        m_f_mat = np.mean(subjects_matrix, axis=0) #compute subject-wise mean for each and every tract
        ...
        for subject_idx in range(subjects_matrix.shape[0]):
            _dropped_matrix = np.delete(subjects_matrix,subject_idx, axis=0) #drop the subject
            m_f_mat = np.mean(_dropped_matrix, axis=0) #compute the mean along all other subject, for each tract
            s, pinv_s = norm_covar_inv(_dropped_matrix) #compute tract-wise covariance
    
            diff_mat = (subjects_matrix[subject_idx,:] - m_f_mat)
            dist = (np.dot(diff_mat, pinv_s) * diff_mat)
            all_dist[subject_idx,:] = dist
    if return_raw:
        results = {'all_dist': np.array(all_dist).sum(axis=1), "raw_dist": all_dist}
    else:
...
def mah_dist_mat_2_roi(feature_mat, roi_feature_vec, pinv_s, return_raw=False):
    """
    Calculates voxelwise D2 values between feature_mat and roi_feature_vec with the option of returning raw distances.
    Args:
...
def voxel2voxel_dist(subdir, suffix_name_comp=".nii.gz", mask_f=None, mask_img=None,
                mask_threshold=None,feat_sub=[], return_raw=False):
        '''
        ...
        my_subject_features_path, my_subject_features = feature_list(subdir, suffix_name_comp, feat_sub)
        comp_image_fname_list = []
        comp_f_list = []


        _submat, _submask, _matmask = feature_gen(feature_image_fname_list=my_subject_features_path,mask_image_fname=mask_f, mask_image=mask_img, mask_threshold=0.9)
        ...
        for _vox in range(_submat.shape[0]):
                _dict = mah_dist_mat_2_roi(_submat, _submat[_vox], pinv_s,return_raw=return_raw)
